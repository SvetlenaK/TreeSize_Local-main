...
FOLDER_OUT_LOG = get_param_ini("FOLDER_OUT_LOG", FILE_INI)
LOCAL_COMP_FILENAME_LASTRUN  = get_param_ini("LOCAL_COMP_FILENAME_LASTRUN", FILE_INI)
HOSTNAME_GCP = get_param_ini("HOSTNAME_GCP", FILE_INI)
PORT_GCP = get_param_ini("PORT_GCP", FILE_INI)
USER_GCP = get_param_ini("USER_GCP", FILE_INI)
PASS_GCP = get_param_ini("PASS_GCP", FILE_INI)
REMOTE_PATH_GCP = get_param_ini("REMOTE_PATH_GCP", FILE_INI)
...
if not os.path.isdir(FOLDER_OUT):
    logging.error(f'FOLDER_OUT: {FOLDER_OUT} => NOT EXIST')
    sw_break_run = 1

if not os.path.isdir(FOLDER_OUT_LOG):
    logging.error(f'FOLDER_OUT_LOG: {FOLDER_OUT_LOG} => NOT EXIST')
    sw_break_run = 1
